                        ...
                        {orgBudgets.map(org => (
                            <div key={org.scope_id} className="mb-4">
                                <div className="flex justify-between items-center bg-gray-900/50 p-3 rounded-lg border border-gray-700">
                                    ...
                                    <div className="text-emerald-400 font-mono">\${org.spent_usd.toFixed(2)} / \${org.budget_usd.toFixed(2)}</div>
                                </div>
                                    ...
                                    {deptBudgets.map(dept => (
                                        <div key={dept.scope_id} className="mb-3">
                                            <div className="flex justify-between items-center text-gray-300 py-1">
                                                ...
                                                <div className="text-emerald-500/80 font-mono text-sm">\${dept.spent_usd.toFixed(2)} / \${dept.budget_usd.toFixed(2)}</div>
                                            </div>
                                            
                                            <div className="ml-6 mt-1 border-l border-gray-700/50 pl-4 space-y-1">
                                                {/* In a real app we'd map agents to depts in the DB, but here we just list all agents for demo purposes since we don't have a relational map yet */}
                                                {agentBudgets.map(agent => (
                                                    <div key={agent.scope_id} className="flex justify-between items-center text-sm text-gray-400 py-1">
                                                        ...
                                                        <div className="font-mono text-xs">\${agent.spent_usd.toFixed(4)} / \${agent.budget_usd.toFixed(2)}</div>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
                            ...
                            <tbody className="bg-gray-800 divide-y divide-gray-700">
                                ...
                                {stats.rules.map((r, i) => (
                                    <tr key={i} className="hover:bg-gray-700/50 transition-colors">
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{new Date(r.timestamp || r.created_at).toLocaleString()}</td>
...
dashboardApp.get('/api/stats', async (c: any) => {
  ...
  try {
    ...
    const budgetRows = await env.DB.prepare('SELECT scope, scope_id, spent_usd, budget_usd FROM scp_budget_ledger WHERE period=?').bind(period).all();
    if (budgetRows && budgetRows.results) {
      budgets = budgetRows.results;
    }
  } catch (e) {}
  ...
  let overrides = 0;
  try {
    const rulesRes = await env.DB.prepare('SELECT * FROM scp_policy_log ORDER BY timestamp DESC, created_at DESC LIMIT 20').all();
